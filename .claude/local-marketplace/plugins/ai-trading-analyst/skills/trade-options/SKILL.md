...
Generate a self-contained HTML file named `TRADE-OPTIONS-<TICKER>.html` directly (do NOT produce a `.md` file).
...
3. Match the visual styling used by `~/.claude/skills/trade/scripts/generate_trade_html.py` (the `trade-report-pdf` skill) so the look-and-feel is consistent across the AI Trading Analyst suite. Reuse the same CSS variables and class conventions:
   ...
   - Pills/badges for signals using the color tokens above.
...
- Cover header: title `Options Analysis: <TICKER> — <COMPANY NAME>`, subtitle with generation date, current price, market cap, next earnings; followed by the disclaimer banner; followed by the TOC.
- `Volatility Dashboard` (IV profile table, IV term structure table, assessment paragraph)
- `Expected Move` (timeframe table, earnings move context if applicable)
- `Options Flow & Sentiment` (P/C ratios, max pain, unusual activity, flow interpretation)
- `Recommended Strategies` (each as an `<article class="stock-card">`-style card, with trade-details table, risk/reward table, P/L scenarios table, management rules list)
- `Strategy Comparison Table`
- `Earnings Play` (only if earnings within 30 days; otherwise a brief note)
- `Options Risk Warnings`
- `Key Levels for Options Traders` table and `Catalyst Calendar`
...
(The block above is the Markdown skeleton — render it as styled HTML and write to `TRADE-OPTIONS-<TICKER>.html`. Do not save the Markdown to disk.)
...
7. **HTML output only.** The deliverable is `TRADE-OPTIONS-<TICKER>.html` — a single self-contained HTML file using the styling conventions of the `trade-report-pdf` skill. Do NOT write a Markdown (`.md`) file. Inline all CSS, escape all dynamic content, and include `@media print` rules.
