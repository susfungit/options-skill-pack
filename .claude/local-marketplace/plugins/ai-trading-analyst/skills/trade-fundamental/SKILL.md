...
Generate a self-contained HTML file named `TRADE-FUNDAMENTAL-<TICKER>.html` directly (do NOT produce a `.md` file).

**Implementation approach:**
1. Compose the analysis content using the Markdown skeleton below as the structural template (sections, tables, ordering, copy).
2. Render the final output as a single self-contained HTML5 document using the `Write` tool. Inline all CSS — no external assets, no JavaScript dependencies, no remote fonts/images.
3. Match the visual styling used by `~/.claude/skills/trade/scripts/generate_trade_html.py` (the `trade-report-pdf` skill) so the look-and-feel is consistent across the AI Trading Analyst suite. Reuse the same CSS variables and class conventions:
   - `:root { --primary:#1a365d; --strong-buy:#22763d; --buy:#48bb78; --hold:#d69e2e; --caution:#dd6b20; --avoid:#c53030; --bg:#fff; --text:#2d3748; --border:#e2e8f0; --disclaimer-bg:#fffff0; --muted:#718096; --panel:#f7fafc; }`
   - Container: `.container { max-width:1100px; margin:0 auto; padding:32px 28px 80px; }`
   - Cover header with `header.cover` (4px solid `--primary` bottom border), prominent disclaimer banner using `--disclaimer-bg`/`--hold` border, and an anchored TOC (`nav.toc`) linking to each section.
   - Section titles styled as `h2.section-title` (color `--primary`, bottom border).
   - Tables: full-width with `border-collapse: collapse`, `th` background `--panel`, `td/th` 1px `--border` borders, padding 8-10px.
   - Render the Fundamental Score as a large badge (matching the `score-badge` styling in `generate_trade_html.py`), and the 5 sub-dimension scores as horizontal CSS bar elements (label + percentage-width fill bar) using `--primary`.
   - Pills/badges for verdicts (Undervalued/Fair Value/Overvalued, Wide/Narrow/None, Strong/Adequate/Weak) using the color tokens above.
   - `@media print` rules so the file prints cleanly to PDF from a browser.
4. Include `<meta charset="utf-8">` and `<meta name="viewport" content="width=device-width,initial-scale=1">`.
5. HTML-escape all dynamic data (ticker, prices, names, narrative text) to prevent broken markup.
6. End with a footer containing the disclaimer and generation date.

**Required HTML sections (in order):**
- Cover header: title `Fundamental Analysis: <TICKER> — <COMPANY NAME>`, subtitle with generation date, market cap, sector, industry; followed by the disclaimer banner; followed by the TOC.
- `Fundamental Score` (large badge + 5 sub-dimension bars + signal pill)
- `Company Overview`
- `Valuation Analysis` (with verdict pill)
- `Growth Analysis` (with verdict pill)
- `Profitability Analysis` (with verdict pill)
- `Financial Health` (with verdict pill)
- `Competitive Moat` (with verdict pill)
- `Management Quality` (with verdict pill)
- `Key Metrics Dashboard` table
- `Fundamental Strengths` (numbered list)
- `Fundamental Weaknesses` (numbered list)
- `Fair Value Estimate` (bull/base/bear table)
- Footer with disclaimer

**Use this Markdown skeleton as the source of structure and copy** — convert it to HTML with the styling above:
...
(The block above is the Markdown skeleton — render it as styled HTML and write to `TRADE-FUNDAMENTAL-<TICKER>.html`. Do not save the Markdown to disk.)
...
8. **HTML output only.** The deliverable is `TRADE-FUNDAMENTAL-<TICKER>.html` — a single self-contained HTML file using the styling conventions of the `trade-report-pdf` skill. Do NOT write a Markdown (`.md`) file. Inline all CSS, escape all dynamic content, and include `@media print` rules. (Exception: when invoked as a subagent by `trade-analyze`, follow the orchestrator's response format and do not write any file.)
