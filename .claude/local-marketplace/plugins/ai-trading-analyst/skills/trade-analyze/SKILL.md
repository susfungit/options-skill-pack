...
**Step 4 — Auto-Generate Per-Ticker HTML Report**

After the markdown file is written, automatically generate a per-ticker HTML report `TRADE-ANALYSIS-<TICKER>.html` so the user has a browser-friendly view of THIS analysis. The HTML covers ONLY the ticker just analyzed — DO NOT consolidate other `TRADE-*.md` files in the directory.

Do this inline — DO NOT invoke the `trade-report-pdf` skill. Work directly:

1. Build a JSON payload containing a single entry in `analyses` for the ticker just analyzed, with `ticker`, `company_name`, `analysis_date`, `trade_score`, `trade_grade`, `trade_signal`, sub-scores, `price_at_analysis`, `price_target`, `stop_loss`, `risk_reward_ratio`, `bull_case`, `bear_case`, `key_levels`, `catalyst`, and `position_size_pct`. Synthesize a single-ticker `executive_summary` (signal grouping with just this ticker, top conviction = this ticker, biggest_risk_flag derived from its risk score, upcoming catalysts from its calendar).
2. Set `report_metadata.report_type` to `"Single-Ticker Analysis Report — <TICKER>"`.
3. Write the payload to `/tmp/trade_report_data.json` using the Write tool.
4. Set the output path env var so the script writes a per-ticker file (NOT the consolidated `TRADE-REPORT.html`):
   ```bash
   TRADE_HTML_OUT="TRADE-ANALYSIS-<TICKER>.html" python3 ~/.claude/skills/trade/scripts/generate_trade_html.py
   ```
5. Clean up the temp file:
   ```bash
   rm -f /tmp/trade_report_data.json
   ```

If `~/.claude/skills/trade/scripts/generate_trade_html.py` is missing, create it per the spec in the trade-report-pdf SKILL.md (stdlib only, self-contained HTML, inlined CSS) and ensure it honors the `TRADE_HTML_OUT` env var (defaulting to `TRADE-REPORT.html` when unset).

If HTML generation fails, note the error to the user but DO NOT block the run — the markdown file is the primary artifact.

Note: the consolidated multi-ticker `TRADE-REPORT.html` is generated only when the user explicitly runs `/trade report-pdf`.

**Step 5 — Confirm Output**

After writing both files, display a summary in the terminal:
- The Trade Score, Grade, and Signal
- The path to the markdown report (`TRADE-ANALYSIS-<TICKER>.md`)
- The path to the per-ticker HTML report (`TRADE-ANALYSIS-<TICKER>.html`)
- Mention that the consolidated multi-ticker view is available via `/trade report-pdf`
...
9. ALWAYS auto-generate a per-ticker `TRADE-ANALYSIS-<TICKER>.html` after writing the markdown file (Phase 3 Step 4). It must contain ONLY the ticker just analyzed — do NOT consolidate other tickers' files. The multi-ticker consolidated `TRADE-REPORT.html` is generated only by `/trade report-pdf`.
